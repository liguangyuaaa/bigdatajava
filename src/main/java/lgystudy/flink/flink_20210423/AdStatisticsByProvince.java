package lgystudy.flink.flink_20210423;
...
public class AdStatisticsByProvince {
    public static void main(String[] args) throws Exception {
        ...
        //1. 从文件中读取数据
        URL resourc = AdStatisticsByProvince.class.getResource("/AdClickLog.csv");
        DataStream<AdClickEvent> adClickEventStream = env.readTextFile(resourc.getPath())
                .map(line -> {
        ...
        SingleOutputStreamOperator<AdCountViewByProvince> adCountResultStream = filterAdClickStream
                ...
                .timeWindow(Time.hours(1), Time.minutes(5))//定义滑窗，5分组输出一次
    ...
    public static class AdCountResult implements WindowFunction<Long, AdCountViewByProvince, String, TimeWindow>{
        ...
        @Override
        public void apply(String s, TimeWindow window, Iterable<Long> input, Collector<AdCountViewByProvince> out) {
    ...
    public static class FilterBlackListUser extends KeyedProcessFunction<Tuple, AdClickEvent, AdClickEvent>{
        ...
        //定义状态，保持当前用户对某一广告的点击次数
        ValueState<Long> countState;
        //定义一个标志状态，保持当前用户是否已经被发送到了黑名单里
        ...
        public void processElement(AdClickEvent adClickEvent, Context context, Collector<AdClickEvent> collector) throws Exception {
            ...
            //判断当前用户对同一个广告的点击次数，如果不够上限，就count加1正常输出；如果达到上限，直接过滤掉，并侧输出流输出黑名单报警
            //首先获取当前的count值
