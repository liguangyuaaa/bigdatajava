package lgystudy.flink.flink_20210508;

import day1_10_20210403.java.Order;
import lgystudy.flink.flink_20210506.OrderEvent;
...
public class TxPayMatch {
    ...
    private final static OutputTag<ReceiptEvent> unmatcheReceipts = new OutputTag<ReceiptEvent>("unmatched-receipts"){};
    public static void main(String[] args) throws Exception{
        ...
        //读取数据并转换成POJO类型
        //读取订单支付事件数据
        URL a = TxPayMatch.class.getResource("/OrderLog.csv");
        System.out.println(a.getPath());

        URL orderResource = TxPayMatch.class.getResource("/OrderLog.csv");
        DataStream<OrderEvent> orderEventDataStream = env.readTextFile(orderResource.getPath())
                .map(line -> {
        ...
        SingleOutputStreamOperator<ReceiptEvent> receipEventStream = env.readTextFile(receiptResource.getPath())
                .map(line -> {
                    String[] fields = line.split(",");
        ...
        //将两条流进行连接合并，进行匹配处理，不匹配的事件输出到侧输出流
        SingleOutputStreamOperator<Tuple2<OrderEvent, ReceiptEvent>>  resultStream = orderEventDataStream.keyBy(OrderEvent::getTxId)
                .connect(receipEventStream.keyBy(ReceiptEvent::getTxId))
                .process(new TxPayMatchDetect());
        resultStream.print("match-pays");
        resultStream.getSideOutput(unmatchedPays).print("unmatched-pays");
        resultStream.getSideOutput(unmatcheReceipts).print("unmatched-receipts");

        env.execute("tx match detect job");

    }
    //实现自定义CoProcessFuntion
    public static class TxPayMatchDetect extends CoProcessFunction<OrderEvent, ReceiptEvent, Tuple2<OrderEvent, ReceiptEvent>>{
        ...
        public void onTimer(long timestamp, OnTimerContext ctx, Collector<Tuple2<OrderEvent, ReceiptEvent>> out) throws Exception {
            //定时器触发，有可能是有一个事件没来，不匹配，也有可能都来过了，已经输出并清空状态
            //判断哪个不为空，那么另一个就没来
            if(payState.value() != null){
                ctx.output(unmatchedPays, payState.value());
            }
            if (receiptState.value() != null) {
                ctx.output(unmatcheReceipts, receiptState.value());
